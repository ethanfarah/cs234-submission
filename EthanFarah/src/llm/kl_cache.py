...
class KLCache:
    ...
    Always uses forward KL: KL(full || compressed).
    ...
    def cache_full_prompt(self, prompt: Prompt) -> None:
        ids = prompt.llm_token_ids if prompt.llm_token_ids is not None else prompt.token_ids
        self._cached_logits = self.llm.get_logits(ids.unsqueeze(0)).squeeze(0)
    ...
    def compute_kl(
        ...
        """Compute per-token forward KL divergence over kept positions.

        Args:
            compressed_ids: shape (1, compressed_len) — batch size must be 1.
            keep_mask: shape (seq_len,) bool — must match cached sequence length.
            attention_mask: optional (1, seq_len) mask for compressed logits.

        Returns:
            Per-token KL divergence, shape (kept_count,).
        """
        ...
        comp_logits = self.llm.get_logits(compressed_ids, attention_mask=attention_mask).squeeze(0)
        if attention_mask is not None:
            comp_logits = comp_logits[attention_mask.squeeze(0).bool()]
        ...
        if full_logits.shape[0] != comp_logits.shape[0]:
            raise ValueError(
                ...
                f"compressed logits have {comp_logits.shape[0]} tokens"
            )
