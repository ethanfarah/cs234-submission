...
# Load .env from project root so HF_TOKEN, WANDB_API_KEY, etc. are available.
...
class MinRatioEnforcementMode(str, Enum):
    HARD = "hard"
    SOFT = "soft"
    OFF = "off"


class MinRatioSelectionStrategy(str, Enum):
    RANDOM = "random"
    PREFIX = "prefix"


class TerminalRewardDistribution(str, Enum):
    LAST_KEPT = "last_kept"
    ALL_KEPT_MEAN = "all_kept_mean"
...
class RewardConfig:
    ...
    min_compression_ratio: float = 0.3  # env floor: at least this fraction kept (avoids empty prompts + ensures learning signal)
    task_score_scale: float = 1.0  # scale task_score (F1) in additive sparse reward so rewards are in a usable range when F1 is tiny
    reward_bias: float = 0.0  # added to additive sparse reward so reported reward stays positive (reward = scale*F1 - penalty + bias)
    ...
    threshold_warmup_episodes: int = 0
    initial_quality_threshold: float | None = None
    min_ratio_enforcement_mode: MinRatioEnforcementMode = MinRatioEnforcementMode.SOFT
    min_ratio_soft_fraction: float = 0.25
    min_ratio_selection_strategy: MinRatioSelectionStrategy = MinRatioSelectionStrategy.RANDOM
...
class AlgoConfig:
    ...
    terminal_reward_distribution: TerminalRewardDistribution = TerminalRewardDistribution.LAST_KEPT
